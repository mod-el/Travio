					...
					if (isset($_GET['item'])) {
						...
						try {
								...
								$db->delete('travio_packages_departures_routes', ['package' => $id], [
									'joins' => [
										'travio_packages_departures' => [
											'type' => 'INNER',
											'on' => 'travio_packages_departures.id = travio_packages_departures_routes.departure',
											'fields' => ['package'],
										],
									],
								]);
							...
							foreach ($packageData['departures'] as $departure) {
								...
								if ($departureQ) {
									if ($departureQ['duration'] !== $departure['duration'] or json_encode($departureQ['guides']) !== json_encode($departure['departure'] ? $departure['departure']['guides'] : [])) {
